...
const oracleABI = require("../artifacts/contracts/mocks/MockUniswapOracleAbove.sol/MockUniswapOracleAbove.json")
  .abi;
const couponManagerABI = require("../artifacts/contracts/DebtCouponManager.sol/DebtCouponManager.json")
  .abi;
...
async function getBalanceAndSupply(account, tokenAddress) {
  ...
  console.log(
    `Balance of user ${accountAddress} for token ${tokenAddress}: ${balance}`
  );
...
async function burnDollarsForCoupons(account, amountToBurn) {
  try {
    const couponManagerContract = new ethers.Contract(
      addresses.couponManagerAddress,
      couponManagerABI,
      account
    );
    await couponManagerContract.exchangeDollarsForCoupons(
      BigNumber.from(amountToBurn),
      { gasLimit: 5000000 }
    );
  } catch (err) {
    console.log(err);
...
async function approveCouponManageToSpend(account) {
  try {
    ...
    await couponManagerContract.setApprovalForAll(
      addresses.couponManagerAddress,
      true,
      { gasLimit: 5000000 }
    );
...
async function redeemCouponsForDollars(account, couponExpiry, amountToRedeem) {
  try {
    ...
    await couponManagerContract.redeemCoupons(
      BigNumber.from(couponExpiry),
      BigNumber.from(amountToRedeem),
      { gasLimit: 5000000 }
    );
  } catch (err) {
    console.log(err);
...
async function getCouponsReturnedForDollars(account) {
  ...
  const couponsReturned = await couponManagerContract.getCouponsReturnedForDollars(
    BigNumber.from("10")
  );
  console.log(
    "coupons returned is " + JSON.stringify(couponsReturned.toString())
  );
}
...
async function runPriceBelowADollarScenario() {
  try {
    ...
    await setCouponCalculator(
      addresses.configAddress,
      addresses.couponCalculator,
      owner
    );
...
async function getCouponBalance(expiryTimestamp, account) {
  ...
  const couponsReturned = await couponContract.balanceOf(
    account.getAddress(),
    BigNumber.from(expiryTimestamp)
  );
  console.log(
    "coupon balance is " + JSON.stringify(couponsReturned.toString())
  );
}
...
async function runWholeFlow() {
  try {
    ...
    await runPriceBelowADollarScenario().then(async res => {
      while (mintEvents.length < 1) {
        await new Promise(resolve => setTimeout(resolve, 500));
      }
      await runPriceAboveADollarScenario(
        mintEvents[0].expiryTime,
        mintEvents[0].amount
      );
    });
  } catch (err) {
