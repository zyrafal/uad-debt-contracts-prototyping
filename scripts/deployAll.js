...
let configDeployed, debtCouponDeployed, debtCouponManagerDeployed, oracleAboveDeployed, oracleBelowDeployed,
    stabilitasTokenDeployed, couponCalculatorDeployed, mockCouponCalculatorDeployed, mockDollarCalculatorDeployed,
    mockExcessDollarDistributorDeployed, oracleSettableDeployed;
...
async function deployConfig(adminAddress) {
    const StabilitasConfig = new ContractFactory(configArtifact.abi, configArtifact.bytecode, owner)
...
async function deployCoupon(configAddress) {
    const DebtCoupon = new ContractFactory(debtCouponArtifact.abi, debtCouponArtifact.bytecode, owner)
...
async function deployCouponManager(configAddress, couponLengthSeconds) {
    const DebtCouponManager = new ContractFactory(debtCouponManager.abi, debtCouponManager.bytecode, owner)
    debtCouponManagerDeployed = await DebtCouponManager.deploy(configAddress, couponLengthSeconds);
    await debtCouponManagerDeployed.deployed();
    return debtCouponManagerDeployed.address;
}

async function deployOracleAbove() {
    const MockUniswapOracleAbove = new ContractFactory(oracleAbove.abi, oracleAbove.bytecode, owner)
...
async function deployOracleBelow() {
    const MockUniswapOracleBelow = new ContractFactory(oracleBelow.abi, oracleBelow.bytecode, owner)
...
async function deployOracleSettable() {
    const MockUniswapOracleSettable = new ContractFactory(oracleSettable.abi, oracleSettable.bytecode, owner)
...
async function deployStabilitasToken(initialSupply) {
    const MockStabilitasToken = new ContractFactory(stabilitasToken.abi, stabilitasToken.bytecode, owner)
...
async function deployCouponCalculator(configAddress) {
    const CouponsForDollarsCalculator = new ContractFactory(couponCalculator.abi, couponCalculator.bytecode, owner)
    couponCalculatorDeployed = await CouponsForDollarsCalculator.deploy(configAddress);
...
async function deployMockCouponsForDollarsCalculator() {
    const MockCouponsForDollarsCalculator = new ContractFactory(mockCouponCalculator.abi, mockCouponCalculator.bytecode, owner)
...
async function deployMockDollarMintingCalculator() {
    const MockDollarMintingCalculator = new ContractFactory(mockDollarCalculator.abi, mockDollarCalculator.bytecode, owner)
...
async function deployMockExcessDollarDistributor() {
    const MockExcessDollarDistributor = new ContractFactory(mockExcessDollarDistributor.abi, mockExcessDollarDistributor.bytecode, owner)
...
async function doDeployment() {
    ...
    const couponManagerAddress = await deployCouponManager(configAddress, couponLengthSeconds);
    ...
    //mocks
    const couponCalculator = await deployMockCouponsForDollarsCalculator(configAddress);
    const dollarCalculator = await deployMockDollarMintingCalculator(configAddress);
    ...
    await configDeployed.setExcessDollarsDistributor(couponManagerAddress, dollarDistributor);
