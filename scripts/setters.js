...
async function setOracle(configAddress, oracleAddress, account) {
    try {
        ...
        const oracleContract = new ethers.Contract(oracleAddress, oracleABI, account);

        //params dont matter as mock
        console.log("New oracle price is " + await oracleContract.consult(oracleAddress, 1, oracleAddress));
    } catch (err) {
        console.log(err);
    }
}

async function setCouponCalculator(configAddress, couponCalculatorAddress, account) {
    const configContract = new ethers.Contract(configAddress, configABI, account);
